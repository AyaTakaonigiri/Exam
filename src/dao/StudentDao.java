...
public class StudentDao extends Dao {
	...
	String logical_delete = " and LOGICAL_DELETE=true";
	/**
	...
	public List<Student> filter(School school, int entYear, String classNum, boolean isAttend) throws Exception {
		...
		//SQL文の在学フラグ条件
		String conditinIsAttend = "";
		//在学フラグがtrueの場合
		if(isAttend) {
			conditinIsAttend = "and is_attend=true";
		}

		try {
			//プリペアードステートメントにSQL文をセット
			statement = connection.prepareStatement(baseSql + condition + conditinIsAttend + logical_delete + order);
	...
	public List<Student> filter(School school, boolean isAttend) throws Exception {
		...
		try {
			//プリペアードステートメントにSQL文をセット
			statement = connection.prepareStatement(baseSql + conditinIsAttend + logical_delete + order);
			//プリペアードステートメントに学校コードをバインド
			statement.setString(1, school.getCd());
			//プリペアードステートメントを実行
			rSet = statement.executeQuery();
		...
		try {
			...
			if(old==null) {
				...
				statement = connection.prepareStatement(
						"INSERT INTO STUDENT(no, name, ent_year, class_num, is_attend, school_cd, LOGICAL_DELETE) VALUES (?, ?, ?, ?, ?, ?, TRUE)");
	...
	public boolean delete(String no) throws Exception {
		...
		try {
			...
			statement = connection.prepareStatement("update student set LOGICAL_DELETE = FALSE, IS_ATTEND = 'FALSE' where no=?");
			// プリペアードステートメントに学生番号をバインド
			statement.setString(1, no);
			// プリペアードステートメントを実行
			count = statement.executeUpdate();
